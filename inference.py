...
from datasets.dataset import FSAD_Dataset_train, FSAD_Dataset_test, FSAD_Dataset_inference
...
def main():
    ...
    test_dataset = FSAD_Dataset_inference(config.dataset.data_path, class_name=config.dataset.obj, is_train=False, resize=config.dataset.img_size, shot=config.dataset.shot, data_type=config.dataset.data_type)
    ...
    for inference_round in range(config.trainer.inferences):
        ...
        scores_list, test_imgs, gt_list = test(config, models, inference_round, fixed_fewshot_list, test_loader, **kwargs)
...
def test(config, models, cur_epoch, fixed_fewshot_list, test_loader, **kwargs):
    ...
    """The shape of score_map is (83, 224, 224)"""
    return score_map, query_imgs, gt_list
